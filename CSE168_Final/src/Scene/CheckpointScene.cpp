	...
	: Scene()
{
	...
	*/
	...
	model_textures.emplace("texture_specular", new Texture("res/textures/gray.jpg"));
	model_textures.emplace("texture_normal", new Texture("res/textures/blank_normal.jpg"));
	...
	*/
	...
	light1->Translate(glm::vec3(3.0f, 3.0f, 3.0f));
	//light1_col = glm::vec3(0.3f, 0.3f, 1.0f);
	light1_col = glm::vec3(1.0f, 1.0f, 1.0f);

	light2 = new Model("res/models/sphere.obj");
	light2->Scale(glm::vec3(0.0005f));
	light2->Translate(glm::vec3(-4.0f, 1.0f, -3.0f));
	light2_col = glm::vec3(1.0f, 0.3f, 0.3f);
	...
	*/
	...
	glGenTextures(1, &depth_cubemap_2);
	...
	glBindFramebuffer(GL_FRAMEBUFFER, depth_map_buffer_2);
	glFramebufferTexture(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, depth_cubemap_2, 0);
	glDrawBuffer(GL_NONE);
	glReadBuffer(GL_NONE);
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
void CheckpointScene::OnEvent(Event& e)
{
	m_Camera->OnEvent(e);
...
void CheckpointScene::OnUpdate(float dt)
{
	...
	glm::mat4 light_proj_2 = glm::perspective(glm::radians(90.0f), 1.0f, 0.1f, 10000.0f);
	std::vector<glm::mat4> shadowTransforms_2;
	shadowTransforms_2.push_back(light_proj_2 * glm::lookAt(light2->GetPosition(), light2->GetPosition() + glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)));
	shadowTransforms_2.push_back(light_proj_2 * glm::lookAt(light2->GetPosition(), light2->GetPosition() + glm::vec3(-1.0f, 0.0f, 0.0f), glm::vec3(0.0f, -1.0f, 0.0f)));
	shadowTransforms_2.push_back(light_proj_2 * glm::lookAt(light2->GetPosition(), light2->GetPosition() + glm::vec3(0.0f, 1.0f, 0.0f), glm::vec3(0.0f, 0.0f, 1.0f)));
	shadowTransforms_2.push_back(light_proj_2 * glm::lookAt(light2->GetPosition(), light2->GetPosition() + glm::vec3(0.0f, -1.0f, 0.0f), glm::vec3(0.0f, 0.0f, -1.0f)));
	shadowTransforms_2.push_back(light_proj_2 * glm::lookAt(light2->GetPosition(), light2->GetPosition() + glm::vec3(0.0f, 0.0f, 1.0f), glm::vec3(0.0f, -1.0f, 0.0f)));
	shadowTransforms_2.push_back(light_proj_2 * glm::lookAt(light2->GetPosition(), light2->GetPosition() + glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, -1.0f, 0.0f)));
	...
	depthShader->Bind();
	depthShader->SetUniformMat4("shadow_matrices[0]", shadowTransforms_2[0]);
	depthShader->SetUniformMat4("shadow_matrices[1]", shadowTransforms_2[1]);
	depthShader->SetUniformMat4("shadow_matrices[2]", shadowTransforms_2[2]);
	depthShader->SetUniformMat4("shadow_matrices[3]", shadowTransforms_2[3]);
	depthShader->SetUniformMat4("shadow_matrices[4]", shadowTransforms_2[4]);
	depthShader->SetUniformMat4("shadow_matrices[5]", shadowTransforms_2[5]);
	depthShader->SetUniformFloat3("light_pos", light2->GetPosition());
	depthShader->SetUniformFloat("far_plane", 10000.0f);
	...
	lightShader->Bind();
	lightShader->SetUniformMat4("camera", m_Camera->GetCameraMat());
	lightShader->SetUniformFloat3("light_color", light2_col);
