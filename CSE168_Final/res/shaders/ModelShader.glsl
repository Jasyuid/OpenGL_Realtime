...
vec3 CalculateFR(float cosT, vec3 spec, float roughness)
{
	return spec + (max(vec3(1.0 - roughness), spec) - spec) * pow(max(1.0 - cosT, 0.0), 5.0);
}
...
float ShadowCalculation(vec3 fragPos, vec3 normal, vec3 light_pos, vec3 light_dir, samplerCube shadow_map)
{
	...
	{
		float closestDepth = texture(shadow_map, fragToLight + sampleOffsets[i] * diskRadius).r;
		closestDepth *= far_plane;
...
void main()
{
	...
	vec3 FR = CalculateFR(max(dot(normal, viewDir), 0.0), F0, rough);
	vec3 kd = (vec3(1.0) - FR) * (1.0 - spec);
	vec3 diffuse = texture(env_map, normal).rgb * diff;
